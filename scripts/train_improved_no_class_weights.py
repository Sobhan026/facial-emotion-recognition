...
def create_callbacks(
    ...
    """
    Create training callbacks for the improved model without class weights.
    """
...
def train_improved_no_class_weights_model(
    epochs: int = EPOCHS,
    batch_size: int = BATCH_SIZE,
) -> tf.keras.Model:
    """
    Train the improved CNN model without class weights.

    This experiment keeps the improved architecture and data augmentation,
    but disables class weighting to test whether class weights were hurting
    overall performance.
    """

    model_name = "improved_cnn_no_class_weights"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train the improved CNN model without class weights on FER2013."
    )
...
def main() -> None:
    ...
    train_improved_no_class_weights_model(
        epochs=args.epochs,
